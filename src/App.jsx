...
import { playCorrectSound, playWrongSound, playApplause } from "./sounds.js";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const ADVANCE_SECONDS = room.lastResult?.correct ? 5 : 3;
    const tick = () => {
  ...
  const openerAutoplayFiredRef = useRef(null);
  ...
  // auto-odtwarzanie muzyki dla wszystkich, gdy tylko kończy się odliczanie
  useEffect(() => {
    if (screen !== "opener" || openerPhase !== "answering" || !room?.openerCreatedAt) return;
    if (openerAutoplayFiredRef.current === room.openerCreatedAt) return;
    openerAutoplayFiredRef.current = room.openerCreatedAt;
    const t = setTimeout(() => togglePlay(), 150);
    return () => clearTimeout(t);
  }, [screen, openerPhase, room?.openerCreatedAt]);
            ...
            {screen === "roundResult" && room.lastResult && (
              <div className="w-full flex flex-col items-center gap-4">
                ...
                <p style={{ color: "var(--muted)", fontSize: 13 }}>
                  Kolejny gracz za {advanceCountdown ?? (room.lastResult?.correct ? 5 : 3)}…
                </p>
              </div>
            )}
