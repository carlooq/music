...
import { fetchAllSongsFromDb, addSongToDb, updateSongInDb, deleteSongFromDb, migrateBundledLibraryToDb, applyCategoryPatchToDb } from "./songsDb.js";
...
function Vinyl({ spinning, revealed, progress = 0 }) {
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      >
        <circle cx={118} cy={118} r={radius} stroke="rgba(0,229,255,0.15)" strokeWidth={4} fill="none" />
        <circle
          cx={118}
          cy={118}
          r={radius}
          ...
          strokeLinecap="round"
          style={{ transition: "stroke-dashoffset 0.2s linear", filter: "drop-shadow(0 0 6px var(--accent))" }}
        />
      </svg>
      <div
        ...
        style={{
          width: 220,
          height: 220,
          background: "radial-gradient(circle at 35% 30%, #2e1650 0%, #180a2e 60%, #0a0417 100%)",
          boxShadow: "0 20px 45px -15px rgba(0,0,0,0.7), inset 0 0 0 6px #0a0417, 0 0 30px -8px var(--accent2)",
      ...
      >
        {[1, 2, 3, 4].map((n) => (
          <div
            ...
            style={{ width: 220 - n * 34, height: 220 - n * 34, border: "1px solid rgba(0,229,255,0.1)" }}
          />
        ))}
        <div
          className="rounded-full flex items-center justify-center text-center px-2"
          style={{ width: 78, height: 78, background: "var(--accent)", color: "#1a0a14", boxShadow: "0 0 24px -2px var(--accent)" }}
        >
          <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 15, lineHeight: 1 }}>
            {revealed ? "ODKRYTE" : "?"}
          </span>
        </div>
        <div className="absolute rounded-full" style={{ width: 10, height: 10, background: "#0a0417" }} />
      </div>
      <div
        className="absolute"
        style={{
          ...
          width: 90,
          height: 4,
          background: "linear-gradient(90deg, var(--muted), var(--accent2))",
          borderRadius: 4,
...
export default function App() {
      ...
      await runTransaction(db, async (tx) => {
        ...
        if (hasGuess && players.length > 1) {
          ...
          tx.update(ref, {
            ...
            resultAt: Date.now(),
            ...summaryFields,
            ...(hasGuess ? { [`tokens.${data.currentPlayerId}`]: increment(1), [`gameGuesses.${data.currentPlayerId}`]: increment(1) } : {}),
          });
        }
      });
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse 900px 600px at 15% -5%, rgba(255,45,154,0.20), transparent 60%), radial-gradient(ellipse 900px 700px at 100% 10%, rgba(0,229,255,0.16), transparent 55%), radial-gradient(ellipse 800px 600px at 50% 110%, rgba(157,0,255,0.14), transparent 55%), var(--bg)",
        color: "var(--text)",
    ...
    >
      <style>{`
        ...
        :root {
          --bg: #0a0118; --surface: #170a2e; --surface2: #22123f;
          --accent: #ff2d9a; --accent2: #00e5ff; --good: #39ff9a; --bad: #ff3860;
          --text: #f5f1ff; --muted: #a294c9;
        }
        @keyframes spin-record { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        @keyframes neon-pulse {
          0%, 100% { text-shadow: 0 0 8px var(--accent), 0 0 20px var(--accent), 0 0 40px rgba(255,45,154,0.4); }
          50% { text-shadow: 0 0 14px var(--accent), 0 0 32px var(--accent), 0 0 60px rgba(255,45,154,0.6); }
        }
        .neon-title {
          animation: neon-pulse 3s ease-in-out infinite;
        }
        .neon-cyan { text-shadow: 0 0 8px var(--accent2), 0 0 20px var(--accent2), 0 0 40px rgba(0,229,255,0.4); }
        .neon-good { text-shadow: 0 0 8px var(--good), 0 0 20px var(--good); }
        .neon-bad { text-shadow: 0 0 8px var(--bad), 0 0 20px var(--bad); }
        .glow-btn { box-shadow: 0 0 18px -4px var(--accent), 0 2px 0 rgba(0,0,0,0.3); transition: all 0.15s ease; }
        .glow-btn:hover { box-shadow: 0 0 26px -2px var(--accent), 0 2px 0 rgba(0,0,0,0.3); transform: translateY(-1px); }
        .glow-card { box-shadow: 0 0 0 1px rgba(255,45,154,0.15), 0 0 24px -12px rgba(255,45,154,0.5), inset 0 0 40px -30px var(--accent2); }
        .slot-btn { transition: all 0.15s ease; }
        .slot-btn:hover { background: var(--accent) !important; color: #1a1428 !important; box-shadow: 0 0 14px -2px var(--accent); }
        input[type="text"], input[type="number"], input[type="password"], textarea {
          background: var(--surface2); border: 1px solid #3a2a5c; color: var(--text);
          border-radius: 8px; padding: 8px 10px; font-family: 'Space Mono', monospace;
        }
        input:focus, textarea:focus { outline: 2px solid var(--accent2); box-shadow: 0 0 12px -2px var(--accent2); }
      `}</style>

      <div className="w-full flex flex-col items-center" style={{ maxWidth: 720 }}>
        <div className="flex items-center gap-2 mb-1">
          <Music4 size={22} color="var(--accent)" style={{ filter: "drop-shadow(0 0 6px var(--accent))" }} />
          <h1 className="neon-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 38, letterSpacing: 1, color: "var(--accent)" }}>OŚ CZASU MUZYKI</h1>
        </div>
        ...
        {screen === "lobby" && room && (
          <div className="w-full flex flex-col gap-5">
            ...
            <section className="w-full rounded-2xl p-5 flex flex-col items-center glow-card" style={{ background: "var(--surface)", border: "1px solid #2a2340" }}>
              <p style={{ color: "var(--muted)", fontSize: 12, textTransform: "uppercase" }}>Kod pokoju</p>
              <div className="flex items-center gap-2">
                <span className="neon-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 44, letterSpacing: 4, color: "var(--accent)" }}>{roomId}</span>
            ...
            {isHost ? (
              <section className="w-full rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid #2a2340" }}>
                ...
                <button
                  ...
                  className="w-full mt-4 py-3 rounded-xl text-lg font-bold flex items-center justify-center gap-2 glow-btn"
                  style={{ background: "var(--good)", color: "#0d1f1a", fontFamily: "'Bebas Neue', sans-serif", boxShadow: "0 0 18px -4px var(--good)" }}
                >
          ...
          <div className="w-full flex flex-col items-center gap-6">
            {openerPhase === "countdown" ? (
              ...
              >
                ...
                <p className="neon-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 160, color: "var(--accent)", lineHeight: 1 }}>
            ...
            {screen === "roundResult" && room.lastResult && (
              <div className="w-full flex flex-col items-center gap-4">
                <div
                  ...
                  style={{
                    background: room.lastResult.correct ? "rgba(57,255,154,0.1)" : "rgba(255,56,96,0.1)",
                    border: `1px solid ${room.lastResult.correct ? "var(--good)" : "var(--bad)"}`,
                    boxShadow: `0 0 30px -12px ${room.lastResult.correct ? "var(--good)" : "var(--bad)"}`,
                  }}
                >
                  <p
                    className={room.lastResult.correct ? "neon-good" : "neon-bad"}
                    style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 26, color: room.lastResult.correct ? "var(--good)" : "var(--bad)" }}
                  >
        ...
        {screen === "gameover" && room && room.winnerIds && room.winnerIds.length > 0 && (
          <div className="w-full flex flex-col items-center gap-5 text-center">
            <Trophy size={48} color="var(--accent)" style={{ filter: "drop-shadow(0 0 14px var(--accent))" }} />
            <p className="neon-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 34, color: "var(--accent)" }}>
            ...
            {(() => {
              ...
              // najwięcej odgadniętych wykonawców i tytułów w tej rozgrywce
              const guessEntries = Object.entries(room.gameGuesses || {}).filter(([, n]) => n > 0);
              const topGuesser = guessEntries.length ? guessEntries.reduce((a, b) => (b[1] > a[1] ? b : a)) : null;

              if (!fastest && !bestStreak && !topGuesser) return null;
              ...
              return (
                <div className="w-full rounded-2xl p-4" style={{ background: "var(--surface)", border: "1px solid #2a2340" }}>
                  ...
                  <div className="flex gap-3 flex-wrap justify-center">
                    ...
                    {topGuesser && (
                      <StatBox
                        label="🎧 Mistrz zgadywania"
                        value={`${room.players.find((p) => p.id === topGuesser[0])?.name}: ${topGuesser[1]}`}
                      />
                    )}
                  </div>
                </div>
