...
export async function heartbeat(playerId, name, uid, avatarUrl = null, roomId = null) {
  try {
    const ref = doc(db, COLLECTION, playerId);
    await setDoc(ref, { name: name || "Gracz", lastSeen: serverTimestamp(), uid: uid || null, avatarUrl: avatarUrl || null, roomId: roomId || null });
  } catch (e) {
